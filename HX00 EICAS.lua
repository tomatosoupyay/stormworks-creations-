...
function onTick()
    ...
    if temp > 60 and temp < 90 then table.insert(cmsgs,"CAB TEMP") end
    if temp > 90 then table.insert(wmsgs,"CAB FIRE") end
    ...
    if throt > 0.9 and RA < 2.5 then table.insert(wmsgs,"TOFF CONF") end
    ...
    if fmcstatus == 1 then table.insert(imsgs,"*FMC") elseif fmcstatus == 2 or fmcexist ~= 0 then table.insert(wmsgs,"FMC FAULT") end
    if gpwsinhib == 1 then table.insert(cmsgs,"GPWS INHBT") end
    
    if blink2 then -- every 3 seconds cycles to second set. might add more sets soon or switch to a for-loop based system since thats way more efficient.
